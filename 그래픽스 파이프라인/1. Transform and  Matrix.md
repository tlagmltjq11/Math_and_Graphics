...
3차원 공간에서 위치와 방향을 갖는 벡터는 선형변환(Affine 변환)을 통해 **이동, 회전, 스케일**이 가능하다는 것을 위 내용들에서 배울 수 있었다.<br>
...
![삼각함수덧셈정리코사인](https://user-images.githubusercontent.com/43705434/120444692-d2a48080-c3c2-11eb-96a9-a76363831d02.PNG)<br>

위처럼 **내적을 통한 사잇각 이용해서** cos(a-b) = cosacosb + sinasinb 라는 식을 얻을 수 있다.<br>
cos(a+b)는 cos(a - (-b))로 취급할 수 있기에 이를 풀어보면 cos(a+b) = cosacosb - sinasinb가 된다.<br>
-> sin세타 = -sin세타 인 이유는 음의 각도로 가게되면 y값은 -가 되기 때문 반면 cos은 여전히 양수다.<br>
<br>
...
sin같은 경우 그래프를 확인해보면 cos과 파이/2 만큼 차이난다는 것을 알 수 있다.<br>
이 점을 이용해서 sin(a + b)를 풀어보면 cos(파이 / 2 - (a+b))가 되고 이 식은 cos((파이/2 -a) -b)가 된다.<br>
이 식을 위에서 구한 공식처럼 풀어내면 cos(파이/2 -a)cosb + sin(파이/2-a)sinb가 된다.<br>
여기서 cos(파이/2 -a)는 sina가 되고 sin(파이/2-a)는 cosa가 된다. (파이/2 차이기 때문)<br>
최종적으로 sin(a+b) = sinacosb + cosasinb가 된다.<br>
<br>

sin(a-b) = sin(a - (-b))이기 때문에 sinacos(-b) + cosasin(-b)가 된다.<br>
이 식은 sinacosb - cosasinb로 나타낼 수 있게 된다.<br>
...
스케일을 되돌리기 위해 v.normalize / v.magnitude를 곱해주는 것을 볼 수 있다.<br>
